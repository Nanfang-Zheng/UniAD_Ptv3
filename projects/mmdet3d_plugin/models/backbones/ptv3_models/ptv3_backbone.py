...
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict, return_enc_out=False):
        """
        ...
        # Run encoder stage by stage so we can capture enc_channels[-2] output
        enc_point = None
        target_enc_name = f"enc{self.num_stages - 2}"
        for name, module in self.enc._modules.items():
            point = module(point)
            if name == target_enc_name:
                # Shallow copy is enough; tensors are replaced (not mutated) in later stages
                enc_point = Point(point.copy())

        if not self.cls_mode:
            point = self.dec(point)

        if return_enc_out:
            # Fallback if enc_point wasn't captured for any reason
            if enc_point is None:
                enc_point = Point(point.copy())
            return point, enc_point
